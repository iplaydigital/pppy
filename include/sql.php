...
$query = "SELECT main.*,a.* FROM `$tableCampProvince` as main LEFT JOIN `$tableCampProvinceDetail` as a ON a.ID_PROVINCE = main.ID";
$query .= " WHERE main.STATUS = 'Show' AND main.ID_MODEL = '1' AND a.LAG = ? ORDER BY main.ORDER ASC ";
$result = $conn->prepare($query);
$result->bind_param("s", $_SESSION['lag']);
$result->execute();
$result = $result->get_result();
...
while ($line = $result->fetch_assoc()) {
    ...
    $query2 = "SELECT main.*,a.* FROM `$tableCampDistrict` as main LEFT JOIN `$tableCampDistrictDetail` as a ON a.ID_DISTRICT = main.ID";
    $query2 .= " WHERE main.ID_MODEL = '1' AND main.ID_PROVINCE = ? AND a.LAG = ? AND main.STATUS = 'Show' ORDER BY main.ORDER ASC ";
    $result2 = $conn->prepare($query2);
    $result2->bind_param("ss", $line['ID'], $_SESSION['lag']);
    $result2->execute();
    ...
    while ($line2 = $result2->fetch_assoc()) {
        ...
        $result3->bind_param("ss", $line['ID_PROVINCE'], $line2['ID_DISTRICT']);
        $result3->execute();
...
$query = "SELECT main.*,a.* FROM `$tableCampSchool` as main LEFT JOIN `$tableCampSchoolDetail` as a ON a.ID_SCHOOL = main.ID";
$query .= " WHERE main.STATUS = 'Show' AND main.ID_MODEL = '1' AND a.LAG = ? ORDER BY main.ORDER ASC ";
$result = $conn->prepare($query);
$result->bind_param("s", $_SESSION['lag']);
$result->execute();
...
$noCampSchool = '0';
$query = "SELECT * FROM `$tableCampSchool` as main LEFT JOIN `$tableCampSchoolDetail` as a ON a.ID_SCHOOL = main.ID";
...
$arraySlide = array();
$noSlide = '0';
$query = "SELECT main.*,a.* FROM `$tableIndexSlide` as main LEFT JOIN `$tableIndexSlideDetail` as a ON a.ID_SLIDE = main.ID ";
...
$query = "SELECT main.*,a.* FROM `$tableIndexSlideVdo` as main LEFT JOIN `$tableIndexSlideVdoDetail` as a ON a.ID_SLIDE = main.ID ";
$query .= " WHERE main.DEL = '0' AND main.STATUS = 'Show' AND a.LAG = ? ORDER BY main.ORDER DESC ";

$result = $conn->prepare($query);
$result->bind_param("s", $lag);
$result->execute();
$result = $result->get_result();
...
$query = "SELECT main.*, a.TITLE_" . $_SESSION['lagText'] . " as TITLECAT
          ...
          ORDER BY main.DAY DESC, main.ID DESC LIMIT 0, 3";
